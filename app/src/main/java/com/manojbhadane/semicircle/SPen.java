...
public class SPen
{
	...
	/**
	 * Potential performance issue: For every event we allocate a new
	 * InputBuilder object which allocates new Input
	 * objects too. We should utilize an Object Pool design pattern here to make
	 * the garbage collector happy.
	 */
	private void registerTouchListener(View view, final InputHandleListener handler)
	{
		mSPEL.setSPenTouchListener(view, new SPenTouchListener()
		{
			...
			{
				handler.handleEvent(new InputBuilder(view).setInputType(InputType.PEN).setButtonAction(
						ButtonAction.DOWN).setEventTime(event.getEventTime()).build());
			}

			public void onTouchButtonUp(View view, MotionEvent event)
			{
				handler.handleEvent(new InputBuilder(view).setInputType(InputType.PEN).setButtonAction(
						ButtonAction.UP).setEventTime(event.getEventTime()).build());
			}
			...
			public boolean onTouchFinger(View view, MotionEvent event)
			{
				...
				case MotionEvent.ACTION_DOWN:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.FINGER).setInputAction(
							InputAction.DOWN).setEventTime(event.getEventTime()).build());
					break;
				case MotionEvent.ACTION_UP:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.FINGER).setInputAction(
							InputAction.UP).setEventTime(event.getEventTime()).build());
					break;
				default:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.FINGER).setInputAction(
							InputAction.MOVE).setEventTime(event.getEventTime()).build());
					break;
				}
				return false;
			}
			...
			public boolean onTouchPen(View view, MotionEvent event)
			{
				...
				{
				case MotionEvent.ACTION_DOWN:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.PEN).setInputAction(
							InputAction.DOWN).setEventTime(event.getEventTime()).build());
					break;
				case MotionEvent.ACTION_UP:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.PEN).setInputAction(
			...
			public boolean onTouchPenEraser(View view, MotionEvent event)
			{
				...
				{
				case MotionEvent.ACTION_DOWN:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.ERASER).setInputAction(
							InputAction.DOWN).setEventTime(event.getEventTime()).build());
					break;
				case MotionEvent.ACTION_UP:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.ERASER).setInputAction(
	...
	private void registerHoverListener(View view, final InputHandleListener handler)
	{
		mSPEL.setSPenHoverListener(view, new SPenHoverListener()
		{
			public boolean onHover(View view, MotionEvent event)
			{
				...
				{
				case MotionEvent.ACTION_DOWN:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.HOVER).setInputAction(
							InputAction.DOWN).setEventTime(event.getEventTime()).build());
					break;
				case MotionEvent.ACTION_UP:
					handler.handleEvent(new InputBuilder(view).setInputType(InputType.HOVER).setInputAction(
			...
			{
				handler.handleEvent(new InputBuilder(view).setInputType(InputType.HOVER).setButtonAction(
						ButtonAction.DOWN).setEventTime(event.getEventTime()).build());
			}
	...
	private void registerClickListener(final View view, final InputHandleListener handler)
	{
		view.setOnClickListener(new OnClickListener()
		{
			...
			{
				handler.handleEvent(new InputBuilder(view).setInputType(InputType.CLICK).setClickAction(
						ClickAction.CLICK).build());
			}
		});
